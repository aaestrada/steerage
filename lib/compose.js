...
const compose = Async(function *({ store, environment, hooks }) {
    ...
    for (const [name, plugin] of Entries(register)) {
        let toRegister = onregister ? yield onregister(name, Object.assign({}, plugin)) : plugin;

        if (toRegister.enabled === false) {
            continue;
        }

        _plugins.add([toRegister], { before: toRegister.before, after: toRegister.after, group: name });
    }
