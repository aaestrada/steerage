...
const Utils = {
    ...
    async validateAndMergeManifestConfigs(options) {
        ...
        // userConfigPath may contain a path or an array of paths to a user config. If it does, merge it with base config above.
        if (Array.isArray(userConfigPath) && userConfigPath.length) {
            for (const configPath of userConfigPath) {

                const user = await Utils.createConfigResolver({
                    config: configPath,
